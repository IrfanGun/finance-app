...
const applyFilter = () => router.get('/transactions', { from: from.value || undefined, to: to.value || undefined, asset_id: assetId.value || undefined, category_id: categoryId.value || undefined }, { preserveState: true, preserveScroll: true, onSuccess: () => { showFilter.value = false; } });
const clearFilter = () => { from.value = ''; to.value = ''; assetId.value = ''; categoryId.value = ''; applyFilter(); };
</script>
...
<template>
    ...
    <div class="mx-auto min-h-screen w-full max-w-lg bg-white pb-28 text-app-heading">
        ...
        <main class="space-y-4 px-6 pt-5"><div v-if="from || to || assetId || categoryId" class="flex items-center justify-between rounded-xl bg-[#EEF3FF] px-4 py-3 text-xs text-[#2457DA]"><span>Filters applied</span><button class="font-semibold" @click="clearFilter">Clear</button></div><p v-if="!props.transactions.length" class="py-10 text-center text-sm text-[#637083]">No transactions found for this period.</p><article v-for="transaction in props.transactions" :key="transaction.id" class="flex items-center gap-4 border-b border-[#EEF1F5] pb-4"><span class="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-[#F5F7FA]" :class="transaction.type === 'income' ? 'text-[#16815B]' : 'text-[#B34735]'"><component :is="transaction.type === 'income' ? ArrowDown : ArrowUpRight" :size="20" /></span><div class="min-w-0 flex-1"><p class="truncate text-sm font-medium">{{ transaction.title || transaction.note || 'Transaction' }}</p><p class="truncate text-xs text-[#637083]">{{ transaction.category?.name || 'Uncategorized' }} Ã‚Â· {{ transaction.account?.name || 'No asset' }} Ã‚Â· {{ transaction.date }}</p></div><strong class="shrink-0 text-sm" :class="transaction.type === 'income' ? 'text-[#16815B]' : ''">{{ transaction.type === 'income' ? '+' : '-' }} Rp {{ currency(transaction.amount) }}</strong></article></main>
        <div v-if="showFilter" class="fixed inset-0 z-30 flex items-end bg-black/30" @click.self="showFilter = false"><form class="mx-auto w-full max-w-lg rounded-t-[28px] bg-white px-6 pb-8 pt-3 text-[#152338]" @submit.prevent="applyFilter"><div class="mx-auto mb-5 h-1 w-9 rounded-full bg-[#D5DBE5]"></div><div class="flex items-start justify-between"><div><p class="text-xs font-medium text-[#637083]">Filter transactions</p><h2 class="text-[22px] font-semibold">Filter Activity</h2><p class="text-sm text-[#637083]">Find transactions using the filters below.</p></div><button type="button" class="text-2xl text-[#637083]" @click="showFilter = false">Ã—</button></div><div class="mt-5 grid grid-cols-2 gap-3"><label class="text-xs font-medium">From<DatePicker v-model="from" placeholder="Select date" /></label><label class="text-xs font-medium">To<DatePicker v-model="to" placeholder="Select date" /></label></div><label class="mt-4 block text-xs font-medium">Asset<SelectInput v-model="assetId" :options="[{ label: 'All assets', value: '' }, ...props.assets.map((asset) => ({ label: asset.name, value: asset.id }))]" placeholder="All assets" /></label><label class="mt-4 block text-xs font-medium">Category<SelectInput v-model="categoryId" :options="[{ label: 'All categories', value: '' }, ...props.categories.map((category) => ({ label: category.name, value: category.id }))]" placeholder="All categories" /></label><div class="mt-6 grid grid-cols-2 gap-3"><button type="button" class="h-[53px] rounded-xl border border-[#2457DA] text-[#2457DA]" @click="clearFilter">Clear</button><button class="h-[53px] rounded-xl bg-[#2457DA] text-white">Show results</button></div></form></div>
        <MobileBottomNav />
    </div>
</template>
