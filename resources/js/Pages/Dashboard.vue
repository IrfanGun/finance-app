...
<template>
    ...
    <div class="mx-auto flex min-h-screen w-full max-w-lg flex-col bg-app pb-24 text-app-heading">
        ...
        <section class="mt-7 px-6"><div class="flex items-center justify-between"><h2 class="text-lg font-semibold">Accounts &amp; assets</h2><button class="flex items-center text-sm font-medium text-[#2457DA]">See all<ChevronRight :size="16" /></button></div><div v-if="accounts.length" class="-mx-6 mt-4 flex gap-3 overflow-x-auto px-6 pb-1 [scrollbar-width:none] [&amp;::-webkit-scrollbar]:hidden"><article v-for="account in accounts" :key="account.id" class="min-w-[220px] rounded-xl bg-[#F5F7FA] p-4"><div class="flex items-center justify-between text-[#2457DA]"><span class="text-base font-medium">{{ account.name }}</span><Banknote :size="19" /></div><p class="mt-2 text-[23px] font-semibold tracking-tight">Rp {{ formatCurrency(account.current_balance) }}</p><p class="mt-1 text-xs text-[#637083] capitalize">{{ account.type }}</p></article></div><p v-else class="mt-4 text-sm text-[#637083]">No accounts yet.</p></section>
