...
<template>
    ...
    <div class="mx-auto flex min-h-screen w-full max-w-lg flex-col bg-app pb-24 text-app-heading">
        <header class="relative overflow-hidden bg-gradient-to-br from-[#11297A] via-[#1F4AC2] to-[#2969E6] px-6 pb-10 pt-5 text-white">
            ...
            <div class="relative mt-7 grid grid-cols-4 gap-3 text-center text-base"><div v-for="item in shortcuts" :key="item.label"><Link v-if="item.href" :href="item.href" class="block"><span class="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-white text-[#2457DA]"><component :is="item.icon" :size="24" /></span><span class="mt-2 block">{{ item.label }}</span></Link><template v-else><span class="mx-auto flex h-12 w-12 items-center justify-center rounded-xl bg-white text-[#2457DA]"><component :is="item.icon" :size="24" /></span><span class="mt-2 block">{{ item.label }}</span></template></div></div>
        </header>
